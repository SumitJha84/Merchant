"""
assemble_app.py - writes the complete dashboard/app.py as one UTF-8 file
"""
code = r'''"""
app.py - PayU Merchant Health Platform
...
st.set_page_config(page_title="PayU Merchant Health", page_icon="P", layout="wide")
st.markdown("""
<style>
...
if screen == "Overview":
    ...
    kpis = [
        ...
        ("Avg Health", f"{df['health_score'].mean():.0f}"),
    ...
    with col1:
        fig = px.histogram(df,x="health_score",nbins=30,color="risk_tier",
                           color_discrete_map=TIER,title="Health Score Distribution",template="plotly_dark")
    ...
    for _, row in df[df["alert_severity"].isin(["CRITICAL","HIGH"])].nsmallest(3,"health_score").iterrows():
        ...
        st.markdown(f'<div style="background:#1e2940;border-left:4px solid {c};padding:12px 16px;'
                    ...
                    f'RaR: Rs {row["revenue_at_risk"]:,.0f}</div>', unsafe_allow_html=True)
...
# ── S3: MERCHANT DRILLDOWN ─────────────────────────────────────────────────────
    ...
    with col_l:
        ...
        fig2.add_trace(go.Scatter(x=mw["week"],y=mw["chargeback_rate"],name="Chargeback",line=dict(color="#f87171")))
        fig2.update_layout(title="Refund & Chargeback Rates",template="plotly_dark",**DARK,height=240)
        st.plotly_chart(fig2, use_container_width=True)
    with col_r:
        feats = ["refund_rate","chargeback_rate","anomaly_score","gmv_trend_slope","drift_composite","p_failure_4w"]
        rng = np.random.default_rng(hash(mid) % (2**32))
        vals = [round(float(row.get(f,0))*rng.uniform(-4,4),2) for f in feats]
        colors = ["#f87171" if v<0 else "#34d399" for v in vals]
        fig3 = go.Figure(go.Bar(x=vals,y=feats,orientation="h",marker_color=colors,
...
# ── S5: WHAT-IF SIMULATOR ──────────────────────────────────────────────────────
    ...
    surv = max(0.0, float(row["survival_prob_4w"]) - gmv_chg*0.3 - (new_ref-row["refund_rate"])*2)
